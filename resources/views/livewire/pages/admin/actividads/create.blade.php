<?php

use Livewire\Volt\Component;
use App\Models\Actividad;
use Illuminate\View\View;

new class extends Component {
    public $name;
    public $description;

    public function rendering(View $view)
    {
        $view->title('Crear Actividad');
    }

    public function mount() {}

    public function save()
    {
        $validated = $this->validate([
            'name' => 'required',
            'description' => 'required',
        ]);

        $actividad = Actividad::create([
            'name' => $this->name,
            'description' => $this->description,
        ]);

        session()->flash('swal', [
            'icon' => 'success',
            'title' => 'Actividad creada',
            'text' => 'La actividad se ha creado correctamente',
        ]);

        $this->redirect(route('admin.actividads.index'), navigate: true);
    }

    public function cancel()
    {
        $this->redirect(route('admin.actividads.index'), navigate: true);
    }
}; ?>

<div>
    <x-slot name="breadcrumbs">
        <livewire:components.breadcrumb :breadcrumbs="[
            [
                'name' => 'Dashboard',
                'route' => route('admin.dashboard'),
            ],
            [
                'name' => 'Actividades',
                'route' => route('admin.actividads.index'),
            ],
            [
                'name' => 'Crear Actividad',
            ],
        ]" />
    </x-slot>


    <x-container class="lg:py-0 lg:px-6">
        <x-card>
            <form wire:submit.prevent="save">
                <h1 class="text-2xl font-bold">
                    Información de la Actividad
                </h1>
                <p class="text-sm text-gray-600 dark:text-gray-400 py-4">
                    Registre la información de la actividad.
                </p>
                <div class="border-t border-gray-200 dark:border-gray-600"></div>

                @include('livewire.pages.admin.actividads.partials.form', [
                    'showForm' => true,
                    'editForm' => false,
                ])

                <x-slot name="footer">
                    <div class="flex justify-end space-x-2">
                        <x-button wire:click="save" spinner="save" label="Guardar" icon="check"
                            interaction="positive" />
                        <x-button slate label="Cancelar" icon="x-mark" interaction="secondary" wire:click="cancel" />
                    </div>
                </x-slot>
            </form>
        </x-card>
    </x-container>
</div>

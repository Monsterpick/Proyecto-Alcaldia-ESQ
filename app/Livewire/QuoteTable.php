...
{
    use WithExport;
    public string $tableName = 'quote-table';
    ...
    {
        return [
            PowerGrid::exportable(fileName: 'quotes')
    ...
    {
        return Quote::query()
            ->join('customers', function($customers){
                $customers->on('quotes.customer_id', '=', 'customers.id');
            })
            ->select([
                'quotes.id',
                'quotes.date',
    ...
    public function actions(Quote $row): array
    {
        ...
        if(Auth::check() && Auth::user()->can('delete-quote')){

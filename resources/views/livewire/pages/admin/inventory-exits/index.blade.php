...
new #[Layout('livewire.layout.admin.admin'), Title('Salida de Inventario')] class extends Component {
    ...
    public function save()
    {
        try {
            ...
            if ($previousBalance < $this->quantity) {
                $this->dispatch('showAlert', [
                    'icon' => 'error',
                    'title' => '⚠️ Stock Insuficiente',
                    'html' => "No se puede retirar <strong>{$this->quantity} {$product->unit_type}</strong><br><br>📊 Stock actual: <strong>{$previousBalance} {$product->unit_type}</strong><br><br>❌ Cantidad solicitada excede el stock disponible",
                    'timer' => 5000,
                ]);
                return;
            }
            ...
            $this->dispatch('showAlert', [
                ...
                'html' => "<strong>-{$this->quantity} {$product->unit_type}</strong> de '{$product->name}' retirados de {$warehouse->name}<br><br>📊 Stock anterior: {$previousBalance}<br>🔹 Stock nuevo: <strong>{$newBalance}</strong><br><br>📋 Ahora completa el reporte de entrega",
                'timer' => 5000,
                'showConfirmButton' => true,
            ]);

            // Redirigir a crear reporte con los datos de la salida
            return $this->redirect(route('admin.reports.create', [
            ...
            \Log::error('Error en salida de inventario: ' . $e->getMessage());
        }
    }
    ...
    public function with(): array
    {
        return [
            ...
            'exits' => Inventory::with(['product.category', 'warehouse'])
            ...
            <div class="overflow-hidden bg-white shadow-sm dark:bg-gray-800 sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <div class="mb-6 flex items-center justify-between">
                        <h3 class="text-lg font-medium">Registrar Salida</h3>
                        <x-button primary label="Nueva Salida" icon="plus" wire:click="$toggle('showForm')" />
                    </div>

                    @if($showForm)
                    <div class="mb-6 rounded-lg border border-red-200 bg-red-50 p-6 dark:border-red-800 dark:bg-red-900/20">
                        <h4 class="mb-4 text-lg font-semibold text-red-900 dark:text-red-100">
                            <i class="fa-solid fa-arrow-up mr-2"></i>Nueva Salida
                        </h4>
                        ...
                        <form wire:submit="save">
                            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                                ...
                                <div>
                                    <x-input label="Detalle (Opcional)" placeholder="Motivo de salida..." wire:model="detail" />
                                </div>
                            </div>
                            <div class="mt-4 flex justify-end space-x-2">
                                <x-button flat label="Cancelar" wire:click="$toggle('showForm')" />
                                <x-button primary label="Guardar Salida" icon="check" type="submit" spinner />
                            </div>
                        </form>
                    </div>
                    @endif
                            ...
                            <tbody class="divide-y divide-gray-200 bg-white dark:divide-gray-700 dark:bg-gray-800">
                                ...
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                                    <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-500 dark:text-gray-400">
                                        {{ $exit->created_at->format('d/m/Y H:i') }}
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="flex items-center">
                                            <i class="{{ $exit->product->category->icon }} mr-2 text-blue-600"></i>
                                            <span class="text-sm font-medium text-gray-900 dark:text-white">{{ $exit->product->name }}</span>
                                        </div>
                                    </td>
                                    ...
                                    <td class="whitespace-nowrap px-6 py-4 text-center">
                                        <span class="inline-flex items-center rounded-lg bg-red-500 px-4 py-2 text-base font-bold text-white shadow-sm dark:bg-red-600">
                                ...
                                <tr>
                                    <td colspan="5" class="px-6 py-12 text-center">
                                        <i class="fa-solid fa-arrow-up mb-4 text-6xl text-gray-400"></i>
                                        <p class="text-gray-500 dark:text-gray-400">No hay salidas registradas.</p>
                                    </td>
                                </tr>
                                @endforelse
                            </tbody>

...
new class extends Component {
    ...
    {
        if ($purchase->inventories->count() > 0) {
        ...
        try {
            ...
            $this->dispatch('pg:eventRefresh-purchase-table');
        } catch (\Exception $e) {
            $this->dispatch('showAlert', [
                'icon' => 'error',
                'title' => 'Error',
...
<div>
    ...
    @can('create-customer')
        <x-slot name="action">
            <x-button info href="{{ route('admin.purchases.create') }}" wire:navigate>
            ...
            function confirmDelete(purchase_id) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        @this.call('deletePurchase', purchase_id);
                    }
                });
            }
        </script>

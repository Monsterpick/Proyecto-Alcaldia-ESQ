        ...
        <form wire:submit.prevent="save">
            <div class="space-y-4">
                ...
                <div class="lg:flex lg:space-x-4">
                    <x-select class="flex-1" label="Producto" wire:model="product_id"
                        placeholder="Seleccione un producto" :async-data="[
                    'api' => route('api.products.index'),
                    'method' => 'POST',
                    ]" option-label="name" option-value="id" />
                    <div class="flex-shrink-0">
                        <x-button class="w-full lg:mt-6.5 mt-4" label="Agregar" wire:click="addProduct"
                            spinner="addProduct" interaction="positive" icon="plus" />
                    </div>
                </div>
                        ...
                        <tbody>
                            <template x-for="(product, index) in products" :key="product.id">
                                <tr class="border-b">
                                    ...
                                    <td class="py-2 px-4">
                                        <x-input type="number" class="w-20" x-model="product.quantity" />
                                    </td>
                                    <td class="py-2 px-4">
                                        <x-input type="number" class="w-20" x-model="product.price" step="0.01" />
                                    </td>
                                    <td class="py-2 px-4" x-text="(product.price * product.quantity).toFixed(2)"></td>
                                    <td class="py-2 px-4">
                                        <x-mini-button label="Eliminar" x-on:click="removeProduct(index)" icon="trash"
                                            rounded red />
                                    </td>
                                </tr>
                            </template>
                            <template x-if="products.length === 0">
                                <tr>
                                    <td class="text-center text-gray-500 py-4 dark:text-gray-300" colspan="5">No hay

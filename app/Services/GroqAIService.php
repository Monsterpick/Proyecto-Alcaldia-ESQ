...
class GroqAIService
{
    ...
    /**
     * Obtiene el nombre de la institución (Setting o Empresa), default: Alcaldía del Municipio Escuque
     */
    protected function obtenerNombreEmpresa(): string
    {
        try {
            $nombre = \App\Models\Setting::get('name', null);
            if (!empty($nombre)) {
                return $nombre;
            }
        } catch (\Throwable $e) {
            // Setting puede no existir
        }
        try {
            ...
            // Empresa puede no existir en este proyecto
        }

        return 'Alcaldía del Municipio Escuque';
    }

    // ============================================================
    // MENSAJES DE TICKET DE SOPORTE TÉCNICO (NUEVO TICKET)
    // ============================================================

    /**
     * Genera un mensaje de WhatsApp para notificar un nuevo ticket de soporte al Super Admin
     *
     * @param array $datosTicket Información del ticket
     * @return array
     */
    public function generarMensajeTicketSoporte(array $datosTicket): array
    {
        if (!$this->isConfigured()) {
            Log::warning('Groq API no configurada');
            return [
                'success' => true,
                'mensaje' => $this->mensajePredeterminadoTicketSoporte($datosTicket),
                'es_ia' => false,
        ...
        try {
            $prompt = $this->construirPromptTicketSoporte($datosTicket);
            ...
            Log::error('Error generando mensaje ticket de soporte', ['error' => $e->getMessage()]);
            return [
                'success' => true,
                'mensaje' => $this->mensajePredeterminadoTicketSoporte($datosTicket),
                'es_ia' => false,
    ...
    /**
     * Construye el prompt para mensaje de Ticket de Soporte (nuevo ticket)
     */
    protected function construirPromptTicketSoporte(array $datos): string
    {
        $superAdminNombre = $datos['super_admin_nombre'] ?? 'Administrador';
        $usuarioNombre = $datos['usuario_nombre'] ?? 'Usuario';
        $usuarioEmail = $datos['usuario_email'] ?? '';
        $categoria = $datos['categoria'] ?? 'General';
        $asunto = $datos['asunto'] ?? 'Sin asunto';
        $descripcion = $datos['descripcion'] ?? '';
        $prioridad = $datos['prioridad'] ?? 'media';
        $ticketId = $datos['ticket_id'] ?? 'N/A';
        $nombreEmpresa = $this->obtenerNombreEmpresa();

        // Emoji según prioridad
        $emojiPrioridad = match($prioridad) {
            'urgente' => '🔴',
            'alta' => '🟠',
            'media' => '🟡',
            'baja' => '🟢',
        ...
        $prompt = "Eres un asistente profesional del sistema de soporte técnico de {$nombreEmpresa}. ";
        $prompt .= "Genera un mensaje de WhatsApp URGENTE y profesional para notificar a un Super Administrador sobre un nuevo ticket de soporte que requiere su atención.\n\n";

        $prompt .= "## INFORMACIÓN DEL TICKET\n";
        $prompt .= "- Super Admin: {$superAdminNombre}\n";
        $prompt .= "- Ticket ID: #{$ticketId}\n";
        $prompt .= "- Usuario Solicitante: {$usuarioNombre}\n";
        $prompt .= "- Email: {$usuarioEmail}\n";
        $prompt .= "- Categoría: {$categoria}\n";
        $prompt .= "- Asunto: {$asunto}\n";
        $prompt .= "- Prioridad: {$prioridad} {$emojiPrioridad}\n";
        $prompt .= "- Descripción: {$descripcion}\n";

        $prompt .= "\n## INSTRUCCIONES\n";
        $prompt .= "1. El mensaje debe ser profesional, urgente y directo\n";
        $prompt .= "2. Saluda al Super Admin por su nombre\n";
        $prompt .= "3. Informa que hay un NUEVO TICKET asignado que requiere atención\n";
        $prompt .= "4. Incluye el ID del ticket con formato *#ID*\n";
        $prompt .= "5. Muestra los datos más importantes: Usuario, Categoría, Asunto, Prioridad\n";
        $prompt .= "6. Si la prioridad es 'urgente' o 'alta', enfatiza la urgencia\n";
        $prompt .= "7. Incluye un resumen breve de la descripción (máximo 100 caracteres)\n";
        $prompt .= "8. Invita a revisar el sistema para más detalles\n";
        $prompt .= "9. Usa emojis de forma moderada (máximo 4)\n";
        $prompt .= "10. El mensaje debe ser CORTO pero completo (máximo 600 caracteres)\n";
        $prompt .= "11. Usa formato WhatsApp: *negritas*, _cursivas_\n";
        $prompt .= "12. NO uses HTML ni código\n";
        $prompt .= "13. Usa el emoji {$emojiPrioridad} para la prioridad\n";
        $prompt .= "14. El tono debe ser de alerta profesional\n\n";
    ...
    /**
     * Genera mensaje para el director cuando llega nueva solicitud (sin número de ticket).
     * Solo: categoría, tipo solicitud, servicio y descripción.
     */
    public function generarMensajeNuevaSolicitudDirector(array $datos): array
    {
        if (!$this->isConfigured()) {
            return [
                'success' => true,
                'mensaje' => $this->mensajePredeterminadoNuevaSolicitudDirector($datos),
                'es_ia' => false,
            ];
        }

        try {
            $prompt = "Eres un asistente de la Administración Pública Municipal. Genera un mensaje de WhatsApp para notificar a un Director sobre una nueva solicitud ciudadana.\n\n";
            $prompt .= "Categoría: " . ($datos['categoria'] ?? 'General') . "\n";
            $prompt .= "Tipo de solicitud: " . ($datos['tipo_solicitud'] ?? $datos['categoria'] ?? 'N/A') . "\n";
            $prompt .= "Servicio/Departamento: " . ($datos['servicio'] ?? 'N/A') . "\n";
            $prompt .= "Descripción: " . ($datos['descripcion'] ?? '') . "\n\n";
            $prompt .= "INSTRUCCIONES: Genera un mensaje breve, profesional, sin número de ticket. Incluye categoría, tipo de solicitud, servicio y descripción. Máximo 500 caracteres. Usa formato WhatsApp (*negritas*). No incluyas ID, ticket ni referencia numérica.";
            $response = $this->llamarGroqAPI($prompt);

            if (!$response['success'] || strlen(trim($response['contenido'] ?? '')) > 600) {
                return [
                    'success' => true,
                    'mensaje' => $this->mensajePredeterminadoNuevaSolicitudDirector($datos),
                    'es_ia' => false,
            ...
            Log::error('Error generando mensaje nueva solicitud director', ['error' => $e->getMessage()]);
            return [
                'success' => true,
                'mensaje' => $this->mensajePredeterminadoNuevaSolicitudDirector($datos),
                'es_ia' => false,
    ...
    protected function mensajePredeterminadoNuevaSolicitudDirector(array $datos): string
    {
        $categoria = $datos['categoria'] ?? 'General';
        $tipoSolicitud = $datos['tipo_solicitud'] ?? $categoria;
        $servicio = $datos['servicio'] ?? 'N/A';
        $descripcion = $datos['descripcion'] ?? '';

        $mensaje = "📋 *Nueva solicitud ciudadana*\n\n";
        $mensaje .= "🏷️ *Categoría:* {$categoria}\n";
        $mensaje .= "📌 *Tipo de solicitud:* {$tipoSolicitud}\n";
        $mensaje .= "🏛️ *Servicio:* {$servicio}\n";
        $mensaje .= "📝 *Descripción:*\n" . \Illuminate\Support\Str::limit($descripcion, 400) . "\n\n";
        $mensaje .= "Por favor revise en el sistema para gestionarla.";
        return $mensaje;
    }

    /**
     * Genera mensaje para el director cuando aprueba o rechaza (confirmación de su acción).
     */
    public function generarMensajeResolucionDirector(array $datos): array
    {
        $nombreCiudadano = $datos['nombre_ciudadano'] ?? 'ciudadano';
        $estado = $datos['estado'] ?? 'aprobado';
        $accion = $estado === 'aprobado' ? 'aprobada' : 'rechazada';

        $mensaje = "✅ *Confirmación*\n\n";
        $mensaje .= "La solicitud de *{$nombreCiudadano}* la has {$accion} correctamente.";
        return ['success' => true, 'mensaje' => $mensaje, 'es_ia' => false];
    }

    /**
     * Mensaje predeterminado para Ticket de Soporte (nuevo ticket)
     */
    protected function mensajePredeterminadoTicketSoporte(array $datos): string
    {
        $superAdminNombre = $datos['super_admin_nombre'] ?? 'Administrador';
        $usuarioNombre = $datos['usuario_nombre'] ?? 'Usuario';
        $categoria = $datos['categoria'] ?? 'General';
        ...
        $mensaje = "🎫 *Nuevo Ticket de Soporte Asignado*\n\n";
        $mensaje .= "Hola *{$superAdminNombre}*,\n\n";
        $mensaje .= "Se ha creado un nuevo ticket de soporte en {$nombreEmpresa} que requiere tu atención.\n\n";
        $mensaje .= "📋 *Ticket:* #{$ticketId}\n";
        $mensaje .= "👤 *Usuario:* {$usuarioNombre}\n";
        $mensaje .= "📁 *Categoría:* {$categoria}\n";
        $mensaje .= "📌 *Asunto:* {$asunto}\n";
        $mensaje .= "{$emojiPrioridad} *Prioridad:* " . ucfirst($prioridad) . "\n\n";

        if ($prioridad === 'urgente' || $prioridad === 'alta') {
            $mensaje .= "⚠️ Este ticket requiere *atención prioritaria*.\n\n";
        }

        $mensaje .= "Por favor, revisa el sistema para más detalles y gestionar este ticket.\n\n";
        $mensaje .= "Gracias por tu pronta atención.";

        return $mensaje;
    }

    // ============================================================
    // MENSAJES DE RESOLUCIÓN DE TICKET (cuando se resuelve)
    // ============================================================

    /**
     * Genera un mensaje de WhatsApp para notificar al usuario que su ticket fue resuelto
     *
     * @param array $datosResolucion Información de la resolución del ticket
     * @return array
     */
    public function generarMensajeResolucionTicket(array $datosResolucion): array
    {
        if (!$this->isConfigured()) {
            Log::warning('Groq API no configurada');
            return [
                'success' => true,
                'mensaje' => $this->mensajePredeterminadoResolucionTicket($datosResolucion),
                'es_ia' => false,
                'motivo' => 'API no configurada',
            ];
        }
        ...
        try {
            $prompt = $this->construirPromptResolucionTicket($datosResolucion);
            $response = $this->llamarGroqAPI($prompt);
            ...
            Log::error('Error generando mensaje de resolución de ticket', ['error' => $e->getMessage()]);
            return [
                'success' => true,
                'mensaje' => $this->mensajePredeterminadoResolucionTicket($datosResolucion),
                'es_ia' => false,
    ...
    /**
     * Construye el prompt para mensaje de Resolución de Ticket
     */
    protected function construirPromptResolucionTicket(array $datos): string
    {
        $usuarioNombre = $datos['usuario_nombre'] ?? 'Usuario';
        $ticketId = $datos['ticket_id'] ?? 'N/A';
        $categoria = $datos['categoria'] ?? 'General';
        ...
        $prompt = "Eres un asistente profesional del sistema de soporte técnico de {$nombreEmpresa}. ";
        $prompt .= "Genera un mensaje de WhatsApp POSITIVO y profesional para notificar a un usuario que su ticket de soporte ha sido RESUELTO.\n\n";

        $prompt .= "## INFORMACIÓN DEL TICKET RESUELTO\n";
        $prompt .= "- Usuario: {$usuarioNombre}\n";
        $prompt .= "- Ticket ID: #{$ticketId}\n";
        $prompt .= "- Categoría: {$categoria}\n";
        $prompt .= "- Asunto: {$asunto}\n";
        $prompt .= "- Respuesta/Solución: {$respuesta}\n";

        $prompt .= "\n## INSTRUCCIONES\n";
        $prompt .= "1. El mensaje debe ser ALEGRE, positivo y profesional\n";
        $prompt .= "2. Saluda al usuario por su nombre\n";
        $prompt .= "3. Comunica que su ticket ha sido *RESUELTO* ✅\n";
        $prompt .= "4. Incluye el ID del ticket con formato *#ID*\n";
        $prompt .= "5. Menciona brevemente el asunto del ticket\n";
        $prompt .= "6. Incluye la respuesta/solución proporcionada\n";
        $prompt .= "7. Agradece al usuario por reportar el problema\n";
        $prompt .= "8. Invita a contactar nuevamente si tiene más dudas\n";
        $prompt .= "9. Usa emojis positivos (máximo 4): ✅ 🎉 ✨ 👍\n";
        $prompt .= "10. El mensaje debe ser CORTO pero completo (máximo 700 caracteres)\n";
        $prompt .= "11. Usa formato WhatsApp: *negritas*, _cursivas_\n";
        $prompt .= "12. NO uses HTML ni código\n";
        $prompt .= "13. El tono debe ser celebratorio pero profesional\n";
        $prompt .= "14. Ofrece disponibilidad para futuras consultas\n\n";
        $prompt .= "Genera SOLO el mensaje, sin introducción ni explicación adicional.";
    ...
    /**
     * Mensaje predeterminado para Resolución de Ticket
     */
    protected function mensajePredeterminadoResolucionTicket(array $datos): string
    {
        $usuarioNombre = $datos['usuario_nombre'] ?? 'Usuario';
        $ticketId = $datos['ticket_id'] ?? 'N/A';
        $asunto = $datos['asunto'] ?? 'Sin asunto';
        $respuesta = $datos['respuesta'] ?? 'Tu problema ha sido resuelto exitosamente.';
        $nombreEmpresa = $this->obtenerNombreEmpresa();

        $mensaje = "✅ *¡Ticket de Soporte RESUELTO!*\n\n";
        $mensaje .= "Hola *{$usuarioNombre}*,\n\n";
        $mensaje .= "¡Excelentes noticias! Tu ticket de soporte ha sido *RESUELTO* exitosamente por el equipo de {$nombreEmpresa}. 🎉\n\n";
        $mensaje .= "📋 *Ticket:* #{$ticketId}\n";
        $mensaje .= "📌 *Asunto:* {$asunto}\n\n";
        $mensaje .= "💬 *Solución aplicada:*\n";
        $mensaje .= "{$respuesta}\n\n";
        $mensaje .= "Agradecemos tu paciencia y por reportar este problema. Si tienes alguna duda adicional o necesitas más ayuda, no dudes en contactarnos.\n\n";
